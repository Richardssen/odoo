...
class account_journal(models.Model):
    ...
    def toggle_favorite(self):
        self.write({'show_on_dashboard': not self.show_on_dashboard})
        return False
    ...
    def get_line_graph_datas(self):
        ...
        data = [{'x': short_name, 'y': start_balance, 'name': name}]
        for stmt in bank_stmt:
            ...
            last_balance = data[-1]['y']
            for _ in range(number_day_to_add + 1):
    ...
    def get_bar_graph_datas(self):
        ...
        data = [{'label': _('Past'), 'value': 0.0, 'type': 'past'}]
        ...
        for i in range(-1,4):
            if i == 0:
                ...
                label = (
                    f'{str(start_week.day)}-{str(end_week.day)} '
                    + format_date(
    ...
    def get_journal_dashboard_datas(self):
        ...
        if self.type in ['bank', 'cash']:
            ...
            if account_ids := tuple(
                ...
                amount_field = 'amount_currency' if self.currency_id else 'balance'
    ...
    def open_action(self):
        ...
        [action] = self.env.ref(f'account.{action_name}').read()
        action['context'] = ctx
    ...
    def open_action_with_context(self):
        ...
        if ctx.get('use_domain', False):
            ...
            action['name'] += f' for journal {self.name}'
        return action
