...
def _auto_install_l10n(cr, registry):
    ...
    if country_code := env.user.company_id.country_id.code:
        ...
        if country_code in ['BJ', 'BF', 'CM', 'CF', 'KM', 'CG', 'CI', 'GA', 'GN', 'GW', 'GQ', 'ML', 'NE', 'CD', 'SN', 'TD', 'TG']:
            ...
            [('name', '=', f'l10n_{country_code.lower()}')]
        ):
            module_list.append(f'l10n_{country_code.lower()}')
        else:
            module_list.append('l10n_generic_coa')
        if country_code == 'US':
            module_list.extend(('account_plaid', 'account_check_printing'))
        if country_code in ['US', 'AU', 'NZ', 'CA', 'CO', 'EC', 'ES', 'FR', 'IN', 'MX', 'UK']:
            module_list.append('account_yodlee')

        if europe := env.ref('base.europe', raise_if_not_found=False):
